//
//  DDLoggerClientUDPSocketManager.swift
//  DDLoggerClient_MAC
//
...
typealias SocketDidReceiveHandler = (_ host: String, _ port: UInt16, _ item: DDLoggerClientItem) -> ()

class DDLoggerClientUDPSocketManager: NSObject {
    static let shared = DDLoggerClientUDPSocketManager()
    ...
    func start(hostName:String, port: UInt16) {
        ...
        //发送一条认证信息
        clientSocket.send("DDLoggerClient_auth".data(using: .utf8)!, toHost: self.socketHost, port:  self.socketPort, withTimeout: 600, tag: 1)
    }

}

extension DDLoggerClientUDPSocketManager: GCDAsyncUdpSocketDelegate {
